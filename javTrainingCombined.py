...
x, y = DatasetPrepare.LoadDatasetCustom("L:\\JAV Folder\\Test Frames", "Combined.npy")
...
train_generator = DataGenerator(x_train, y_train.astype(floatType), batch_size = batch_size, dim = input_shape, n_classes=2, shuffle=True)
val_generator = DataGenerator(x_val, y_val.astype(floatType), batch_size= 1, dim = input_shape,n_classes=2, shuffle=False)

# define model
model = sm.Unet(BACKBONE, encoder_weights=encoder_weights, classes = 1, encoder_freeze=encoder_freeze, activation='sigmoid', input_shape=input_shape)
...
if (Load_opt == 1):
  ...
  model.load_weights('CombinedBest.hdf5')
...
import shutil
src="Combined Best Model.hdf5"
dst="Combined Best Model_bak.hdf5"
shutil.copy(src,dst)
